    ...
    <div className="flex flex-row items-center pl-2 pr-4 py-2 hover:bg-muted transition-colors first:rounded-t-xl last:rounded-b-xl">
      ...
      >
        ...
        <div className="text-right ml-4 flex items-center gap-2">
          <MigrationBadge status={contact.migration_status} />
          {contact.last_seen && (
            <div
            ...
            >
              ...
              <LifecycleStageBadge lifecycleStage={contact.lifecycle_stage} />
...
const MigrationBadge = ({ status }: { status?: string }) => {
  if (!status) return null;

  const config: Record<string, { label: string; className: string }> = {
    verified: { label: "Verified", className: "bg-green-500/10 text-green-500" },
    imported: { label: "Imported", className: "bg-yellow-500/10 text-yellow-500" },
  ...
  return (
    <span className={cn("px-1.5 py-0.5 text-[10px] rounded font-medium whitespace-nowrap", c.className)}>
      {c.label}
    </span>
  );
};
...
const LifecycleStageBadge = ({
  ...
  const { lifecycleStages } = useConfigurationContext();
  if (!lifecycleStage || !lifecycleStages) return null;

  const stage = lifecycleStages.find((s) => s.value === lifecycleStage);
  if (!stage) return null;

  const colorMap: Record<string, string> = {
    new_lead: "bg-blue-500/10 text-blue-500",
    active: "bg-green-500/10 text-green-500",
    returning: "bg-purple-500/10 text-purple-500",
  ...
  return (
    <span
      className={cn(
        ...
        colorMap[lifecycleStage] || "bg-gray-500/10 text-gray-500"
      )}
    >
      {stage.label}
    </span>
