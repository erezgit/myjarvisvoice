...
const dataProviderWithCustomMethod: CrmDataProvider = {
  ...
  membersCreate: async ({ ...data }: MemberFormData): Promise<Member> => {
  ...
  membersUpdate: async (
    ...
    const { data: previousData } = await dataProvider.getOne<Member>("members", {
      id,
    });
  ...
  [
    {
      ...
      beforeCreate: async (params) => {
        const { data } = params;
        // If role is not set, default to salesperson
        if (data.role == null) {
          data.role = 'salesperson';
        }
        // Keep administrator in sync with role
      ...
      beforeDelete: async (params) => {
        ...
        const newMemberId = params.meta.identity.id as Identifier;

        const [companies, contacts, contactNotes] = await Promise.all([
          dataProvider.getList("companies", {
            filter: { member_id: params.id },
            pagination: {
              page: 1,
              perPage: 10_000,
            },
