...
import { PermissionInputPanel } from "@/components/desktop/chat/PermissionInputPanel";
import { PlanPermissionInputPanel } from "@/components/desktop/chat/PlanPermissionInputPanel";
import { FileUploadButtonLabel } from "@/components/desktop/chat/FileUploadButtonLabel";
import { FileContextChips } from "@/components/desktop/chat/FileContextChips";
import { VoiceInput, VoiceInputHandle } from "@/components/desktop/chat/VoiceInput";
...
export function ChatInput({
  ...
  const { files: selectedFiles, removeFile } = useSelectedFiles();
  ...
  // Listen for file path insertions from the Files page
  useEffect(() => {
    const handleInsertFile = (e: Event) => {
      const filePath = (e as CustomEvent).detail?.path;
      if (filePath) {
        const prefix = input ? `${input} ` : "";
        onInputChange(`${prefix}${filePath}`);
        textareaRef.current?.focus();
      }
    };
    window.addEventListener('insert-file-path', handleInsertFile);
    return () => window.removeEventListener('insert-file-path', handleInsertFile);
  }, [input, onInputChange]);

  // Global keyboard shortcut for voice input (Ctrl+M / Cmd+M)
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      // Ctrl+M (Windows/Linux) or Cmd+M (Mac) - toggle voice recording
      if (
        e.key.toLowerCase() === KEYBOARD_SHORTCUTS.VOICE_INPUT_TOGGLE &&
        (e.ctrlKey || e.metaKey) &&
        !e.shiftKey && // Shift+Ctrl+M is permission mode toggle
        !isLoading &&
        !showPermissions
      ) {
        e.preventDefault();
  ...
  return (
    <div className="flex-shrink-0">
      {/* File context chips - show selected files above input */}
      <FileContextChips files={selectedFiles} onRemove={removeFile} />
      ...
      <form onSubmit={handleSubmit} className="relative">
        <textarea
          ...
          rows={1}
          className={`w-full px-4 py-3 pr-36 bg-white/80 dark:bg-slate-800/80 border border-neutral-200 dark:border-neutral-700 rounded-xl transition-all duration-200 backdrop-blur-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 resize-none overflow-hidden min-h-[48px] max-h-[${UI_CONSTANTS.TEXTAREA_MAX_HEIGHT}px] outline-none text-base`}
          disabled={isLoading}
        />
        <div className="absolute right-2 bottom-3 flex gap-2">
          ...
          {/* VoiceInput hidden — not functional yet
          <VoiceInput
            ref={voiceInputRef}
          ...
          */}
          ...
          <button
            ...
            className="p-2 bg-neutral-600 hover:bg-neutral-700 disabled:bg-slate-400 text-white rounded-lg transition-all duration-200 shadow-sm hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"
            title={permissionMode === "plan" ? "Plan" : "Send"}
          >
