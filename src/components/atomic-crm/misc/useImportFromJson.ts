...
export type ImportFromJsonFailures = {
  members: Array<JsonTypes.JsonPrimitive | JsonTypes.JsonStruct | undefined>;
...
export const useImportFromJson = (): [
  ...
  const importFile = useEvent(async (file: File) => {
    ...
    const idsMaps: {
      members: Record<number, Identifier>;
    ...
    const importSale = async (
      ...
      try {
        ...
        const existingRecordResponse = await dataProvider.getList("members", {
          filter: { email: dataToImport.email.trim() },
          pagination: { page: 1, perPage: 1 },
          sort: { field: "id", order: "ASC" },
        });
        if (existingRecordResponse.total === 1) {
          idsMaps.members[dataToImport.id] = existingRecordResponse.data[0].id;
          return existingRecordResponse.data[0].id;
        }

        const data = await dataProvider.membersCreate({
        ...
        idsMaps.members[dataToImport.id] = data.id;
        setState((old) => {
          if (old.status === "error") {
            return {
              ...old,
              stats: {
                ...
                members: (old.stats ?? defaultStats).members + 1,
              },
            };
          }
          return {
      ...
      try {
        const { data } = await dataProvider.create("companies", {
          data: {
            ...
            members_id: dataToImport.members_id
              ? idsMaps.members[dataToImport.members_id]
              : currentSale.id,
    ...
    const importNote = async (
      ...
      try {
        if (idsMaps.members[dataToImport.members_id] == null) {
          console.error(
            `note ${dataToImport.text} has an invalid members ID: ${dataToImport.members_id}. Fallback to default member`,
          );
        }
        ...
        await dataProvider.create("contact_notes", {
          data: {
            ...
            members_id: idsMaps.members[dataToImport.members_id] ?? currentSale.id,
    ...
    const importTask = async (
      ...
      try {
        if (idsMaps.members[dataToImport.members_id] == null) {
          console.error(
            `task ${dataToImport.text} has an invalid members ID: ${dataToImport.members_id}. Fallback to default member`,
          );
        }
        if (idsMaps.contacts[dataToImport.contact_id] == null) {
          setState((old) => ({
...
const TYPES = ["members", "companies", "contacts", "notes", "tasks"] as const;
